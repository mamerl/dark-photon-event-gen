...
for sample in sample_list:
    for signal_region in signal_regions:
        ...
        limit_curve[signal_region]["xsec_limits"].append(excluded_xsec)
...
plt.subplots_adjust(hspace=0.05)
ax[1].set_xlabel(r"$m_{Z'}$ [GeV]", fontsize=28)
ax[0].set_ylabel(r"$\sigma \times A \times BR$", fontsize=28)
ax[1].set_ylabel("Ratio", fontsize=28)
...
for signal_region, color in zip(signal_regions, ["C0", "C1"]):
    # for each signal region sort the limit curve by mass
    masses = np.array(limit_curve[signal_region]["masses"])
    limits = np.array(limit_curve[signal_region]["xsec_limits"])
    sorted_indices = np.argsort(masses)
    masses = masses[sorted_indices]
    limits = limits[sorted_indices]
    # mask out any nan values
    ...
    ax[0].plot(
        run2_xsec_limits[signal_region]["mass"],
        run2_xsec_limits[signal_region]["limit"],
        ls="-",
        color=color,
        lw=3,
        marker="o",
    ...
    # calculate the difference between the published and reinterpretation limits
    # at each mass point in the reinterpretation
    run2_masses_mask = np.full_like(run2_xsec_limits[signal_region]["mass"], False, dtype=bool)
    for mass in masses:
        run2_masses_mask |= np.array(run2_xsec_limits[signal_region]["mass"]) == mass
    ratio = limits / np.array(run2_xsec_limits[signal_region]["limit"])[run2_masses_mask]
    ratio[np.isinf(ratio)] = np.nan
...
logger.info("saving signal strength plot to outputs/dmsimp_xsec_exclusion_limits.pdf")
plt.savefig("outputs/dmsimp_xsec_exclusion_limits.pdf")
